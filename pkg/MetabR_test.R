##01291824 1844: all parameters added to GUI, but error saying subscript out of bounds

##01302012 1220: entire program works; next step is to improve layout 
##by organizing some widgets top to bottom within additional containers

##01302012 1643: pasted in new gWidgets code to organize layout.

##01302012 1955: trying to add help buttons 

##02082012 1315: haven't added help buttons; need to remove "(optional)" from tabs; 
##adding check box for either .csv or .txt file

##02082012 1509: added check box for using csv or txt files; added a try() to the aov step to prevent errors from
##insufficient data

##02082012 1616: tried to use pos.good and pos.bad to give pre- and post-norm data the same dimensions

##02082012 1653: removed "complete.cases" in abline in prevpost plotting

##02092012 0200: trying to fix prevpost plotting; metabR plots everything fine when I set parameters by script, but
##not when I use widgets; so I'm setting the widgets equal to their values, and trying the program

##02092012 0258: prevpost seems to be working; the key may have just been typing model terms in quotes in widgets;
##However, I'm not sure it's handling removing/adding terms to random.factors correctly; it actually is handling them
##correctly.  Now, I've noticed that metabR requires two data sets, and if only data 1 is selected, an error is returned.
##So, I'm fixing that.  Now I've fixed this bug by using a try function and if the result is class "try-error", 
##program doesn't try to read in data.neg. 

##02092012 1915: program now exports fold.changes and p.values

##02142012 0326: adding heatmap from normalized data; it works, but need to add parameters for user control

##02152012 1808: adding a check to determine if there are multiple levels of the random-effect variables...if 
##there are not (i.e., metabolite only detected on one run day), then the random-effect variable will be 
##excluded from the model so the metabolite is not added to the "bad.metabolites" list.

##02162012 2010: added check so that if there are not multiple levels of a random-effect variable for 
##a metabolite observed, the random-effect variable will not be included in the linear model

##02162012 2106: now any metabolites that pass lmer but return residuals = 0 will be discarded

##02272012 0230: adding font size options (cex...) to heatmap tab

##02272012 1137: additional heatmap options added; changed widget labels to sound nicer

##02282012 1500: adding "both" option to GUI window so user can opt to have printed only metabolites
##with significant fold-change and p-value; "both" has now been added and works with out error.

##02292012 0844: trying to make the normalization terms into drop-down checklist containing all column headings

##02292012 1258: made fixed.continuous a drop-down checklist using gcheckboxgroup and removed options to pick 
##fixed.categorical variables; now Group is automatically put into the normalization model

##02292012 1551: making random.factors into a drop-down checklist; working now

##02292012 1713: some metabolites are getting by with yielding all residuals equal nearly zero and are then 
##yielding extremely low p-values, whicha are artificial; trying to make a rule to eliminate these

##03012012 1120: changed heatmap colors to blue-black-red; trying to add a "back" button so user doesn't have
##to re-load program in order to run on different data sets; actually that will have to wait for later 
##after I get my GST retreat poster printed

##03042012 1700: trying to fix problem with an error if a second data file is not selected; somehow 
##this is no longer producing an error; working on adding "back" button

##03062012 1039: trying to fix problem when there are no variables selected for normalization; seeing if only
##including "Group" results in the original, un-normalized data; it does yield the exact same original data; changed
##if statement to...if (length(fixed.continuous)!=0)

##03142012 1827: transposed heatmap (columns are now rows, vice versa)

##04092012 1235: zero-centered data for heatmap in order to point out differences 

##04092012 1612: trying to improve prevpost plot so that legend stays out out
##of the way of the plot

##04112012 1111: continuing trying to improve prev.post plot; taking parts of function "legend" in order to
## calculate what y position the top of the legend would be and then adjusting the plot window accordingly		 

##04112012 2330: added code to prevpost plots to place legend in location so that it doesn't overlap any data points

##04122012 1452: prevpost plots finally look good with legend out of the way of data points

##04132012 0906: for some unknown reason, prevpost plot in the first plot made is putting legend 
##flush against top of window, which is not consistent with the others; this is now fixed

##05012012 0000: need to set criteria for number of replicates to minimum of 3, because for n=2 
##the CI bars drawn take into account "n-1" 

##05112012 0301: working on setting criteria for minimum number of replicates to 3; it's working now. Also, I had misinterpreted
#an error in heatmap.2 function.  Missing values are allowed, but only NA and not NaN.  Now NaNs are replaced by NAs
#and the heatmap works with complete data; don't have to remove metabolites with some or too many NAs anymore.  It looks like 
#the clustering is much better.  Lastest version: this criteria occurs right after anova so occuring together are
#criteria that for each metabolite, all p-values are not NaN and at least two treatment groups have at least 3 data points

##05122012 1635: svalue(nb)=1 to make first tab the one that is visible; also added if statement
##to replace NA with 0 if there are too many NAs to draw the heatmap

##05132012 1401: improving mean plots.  For treatment groups with only 3 replicates, they should be replaced with NA
##because CI bars are artificially large and are not valid, and they screw up the plotting window.

##05152012 0929: mean plots are improved.  prevpost isn't 100% adaptive to number of treatment groups.  the plotting isn't working
##currently because I'm working through it to fix it.  I think I need to put legend outside of plotting space, using layout()
##and making it essentially another plotting object in the third space

##05182012 1320: dropped in improved prevplot section, seeing if it works

##05182012 1008: prevplot looks good now, just a little tall and narrow plotting area; forgot to copy in heatmap 
##writing .csv files section, so it is added to this version

##05222012 0001: making same changes to residual plot as for pre vs post normalization--putting legend outside
##of plotting region

##05242012 1424: adding check.names=FALSE to reading in data so numbers won't have "X" 
##placed in front of them.

##05252012 1000: added improved residual plot code to program with better legend

##08082012 0028: added qvalues option

##08122012 1940: qvalues option seems to be working

##08122012 2327: made minor rearrangement to "required" tab GUI layout

##08132012 0039: adding write q-values and print q-values in console

##08202012 1045: there was a weird problem with "as.numeric" not coercing vectors to numeric.
##I had to use "options(stringsAsFactors = FALSE)" to get around it.  This only happened in the 
##mean plot section of the program, number 7, to my knowledge. 

##08232012 1036: adding previously written script that produces a Pathway Projector input file

##08272012 0951: added Pathway Projector script and GUI widgets

##08302012 1857: updated to R 15.1 to check compatibility--qvalue package wasn't found because 
##Bioconductor repository must first be selected; added setRepositories() command so the user
##must now click on "BioC software"

##08312012 0240: added "as.factor" everywhere "levels" was used because an error was returned 
##when MetabR was re-run without re-opening R 
...
fixed.continuous = gcheckboxgroup(colnames(data.pos)[1:(col.start-2)], container=required.1, use.table=TRUE, coerce.with=svalue)

random.factors.label <- glabel("Random-effect variables", container = required.1)

random.factors = gcheckboxgroup(colnames(data.pos)[1:(col.start-2)], container=required.1, use.table=TRUE, coerce.with=svalue)
